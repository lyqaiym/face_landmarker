 ...
 * AudioFormat#ENCODING_PCM_16BIT}) or Short (in {@link
 * AudioFormat#ENCODING_PCM_FLOAT}). Internally it converts and stores all the audio
...
public class AudioData {
  ...
  /**
   * Creates a {@link AudioRecord} instance with a ring buffer whose size is {@code
   ...
   *
   * @param format the {@link AudioFormat} required by the TFLite model. It defines
   ...
   *
   * @param src input audio samples in {@link AudioFormat#ENCODING_PCM_FLOAT}. For
   ...
   * buffer.
   *
   * @param src input audio samples in {@link AudioFormat#ENCODING_PCM_16BIT}. For
   *     multi-channel input, the array is interleaved.
   */
  ...
  /**
   * Loads latest data from the {@link AudioRecord} in a non-blocking way. Only
